...
int main(void)
{
  ...
  MX_UART4_Init();
  /* USER CODE BEGIN 2 */
  HAL_UART_Transmit(&huart4, "Init\n", 5, HAL_MAX_DELAY);

  l3gd20_data_t l3gd20_data = { 0 };

  if (l3gd20_init(&hspi1, L3GD20_SCALE_2000) != L3GD20_OK) {
	  HAL_UART_Transmit(&huart4, "InitERROR\n", 10, HAL_MAX_DELAY);
	  return;
  	}
  HAL_UART_Transmit(&huart4, "InitEND\n", 8, HAL_MAX_DELAY);
  /* USER CODE END 2 */
  ...
  while (1)
  {
	  /* data in l3gd20_data */
	   ...
	   sprintf(msg, "x: %04i -- y: %04i -- z: %04i\n", l3gd20_data.x, l3gd20_data.y, l3gd20_data.z);
